...
function LoginScreen({ onLogin }) {
  ...
  const attempt = () => {
    if (pw === PASSWORDS.editor) { onLogin("editor"); return; }
    if (pw === PASSWORDS.viewer) { onLogin("viewer"); return; }
  ...
  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#0a0a0a",fontFamily:"'Inter','Helvetica Neue',Arial,sans-serif"}}>
      <style>{`@keyframes shake{0%,100%{transform:translateX(0)}20%,60%{transform:translateX(-8px)}40%,80%{transform:translateX(8px)}}`}</style>
      <div style={{textAlign:"center",width:"100%",maxWidth:380,padding:"0 24px"}}>
        <div style={{fontSize:28,fontWeight:800,color:"#ffffff",letterSpacing:"-0.04em",marginBottom:8}}>Garrigosa Studio</div>
        <div style={{fontSize:14,color:"#525252",fontWeight:500,marginBottom:48,letterSpacing:"0.06em",textTransform:"uppercase"}}>Internal Platform</div>
        <div style={{animation:shake?"shake 0.4s ease":"none"}}>
          <input
            ...
            autoFocus
            style={{width:"100%",padding:"14px 18px",fontSize:16,borderRadius:"8px",border:`1.5px solid ${error?"#ef4444":"#2a2a2a"}`,background:"#141414",color:"#ffffff",outline:"none",textAlign:"center",letterSpacing:"0.1em",marginBottom:12,fontFamily:"inherit"}}
          />
          {error && <div style={{fontSize:13,color:"#ef4444",fontWeight:600,marginBottom:12}}>Incorrect password. Try again.</div>}
          <button onClick={attempt} style={{width:"100%",padding:"14px",borderRadius:"8px",border:"none",background:"#ffffff",color:"#0a0a0a",fontSize:15,fontWeight:700,cursor:"pointer",letterSpacing:"-0.01em"}}>
            Enter →
          </button>
        </div>
      </div>
...
const PROJECT_PHASES = ["Brief received","Budgeting","Budget sent","Approved — in production","Client review","Delivered","Archived"];
const PHASE_COLORS = {
...
const SEED_DELIVERABLES = [
  ...
  {id:"try23",project_id:"p2",label:"Render adicional — Cenital SKU 50mg",format:"CGI Still · square",owner:"oscar",status:"Not started",delivery:false,notes:"Top down"},
  {id:"try24",project_id:"p2",label:"Render adicional — Cenital SKU 80mg",format:"CGI Still · square",owner:"oscar",status:"Not started",delivery:false,notes:"Top down"},
  ...
  {id:"so1",project_id:"p6",label:"P1 — KV: Analin smashing GOUT",format:"Photo + CGI · HCP",owner:"alex_t",status:"In progress",delivery:false,notes:"Priority 1 · Shot principal HCP"},
  {id:"so2",project_id:"p6",label:"P2 — KV: Richard smashing GOUT",format:"Photo + CGI · DTC",owner:"alex_t",status:"In progress",delivery:false,notes:"Priority 2 · Shot principal DTC"},
  {id:"so3",project_id:"p6",label:"P3 — CGI: Letters GO revealed",format:"CGI · Extra",owner:"oscar",status:"In progress",delivery:false,notes:"Priority 3 · PSD format"},
  {id:"so4",project_id:"p6",label:"P4.1 — Richard con Nurse standing",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 4"},
  {id:"so5",project_id:"p6",label:"P4.2 — Richard con Nurse · infusion chair",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 4 · background needed"},
  {id:"so6",project_id:"p6",label:"P4.3 — Richard con Nurse sitting",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 4 · background needed"},
  {id:"so7",project_id:"p6",label:"P4.4 — Richard con Male Doctor",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 4"},
  {id:"so8",project_id:"p6",label:"P4.5 — Richard con Analin",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 4"},
  {id:"so9",project_id:"p6",label:"P5.1 — Analin con Richard · tablet",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 5"},
  {id:"so10",project_id:"p6",label:"P6.1 — Nurse con patient IV · sitting",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 6"},
  {id:"so11",project_id:"p6",label:"P6.2 — Nurse con patient IV · standing",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 6"},
  ...
  {id:"so13",project_id:"p6",label:"P7.1 — Richard · standing with confidence",format:"Photo · DTC Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 7"},
  {id:"so14",project_id:"p6",label:"P7.2 — Richard · looking to side",format:"Photo · DTC Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 7"},
  {id:"so15",project_id:"p6",label:"P7.3 — Richard · smiling full body",format:"Photo · DTC Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 7"},
  {id:"so16",project_id:"p6",label:"P7.4 — Richard · stuck behind GOUT",format:"Photo · DTC Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 7"},
  {id:"so17",project_id:"p6",label:"P7.5 — Richard · hand propped",format:"Photo · DTC Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 7"},
  ...
  {id:"so19",project_id:"p6",label:"P8.2 — Analin · looking to side",format:"Photo · HCP Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 8"},
  ...
  {id:"so22",project_id:"p6",label:"P9 — KV: Nurse smashing GOUT",format:"Photo + CGI · HCP",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 9"},
  {id:"so23",project_id:"p6",label:"P10 — KV: Male Doctor smashing GOUT",format:"Photo + CGI · HCP",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 10"},
  {id:"so24",project_id:"p6",label:"P11.1 — Male Doctor · standing",format:"Photo · HCP Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 11"},
  {id:"so25",project_id:"p6",label:"P11.2 — Male Doctor · looking to side",format:"Photo · HCP Portrait",owner:"alvaro",status:"Not started",delivery:false,notes:"Priority 11"},
  ...
  {id:"so27",project_id:"p6",label:"P11.4 — Male Doctor + Consumer",format:"Photo · Crossover",owner:"alex_t",status:"Not started",delivery:false,notes:"Priority 11"},
  ...
  {id:"so30",project_id:"p6",label:"Video — Analin smashing GOUT",format:"Video · HCP",owner:"alex_t",status:"Not started",delivery:true,notes:"HCP Talent 1"},
  {id:"so31",project_id:"p6",label:"Video — Male Doctor smashing GOUT",format:"Video · HCP",owner:"alex_t",status:"Not started",delivery:true,notes:"HCP Talent 2"},
];
...
function Avatar({ memberId, size=32 }) {
  ...
  if (!m) return null;
  return <div style={{width:size,height:size,borderRadius:"50%",background:m.color+"18",border:`1.5px solid ${m.color}35`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:Math.round(size*0.34),fontWeight:700,color:m.color,flexShrink:0,userSelect:"none"}}>{m.initials}</div>;
}
  ...
  if (loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:W,fontFamily:FONT}}>
      <div style={{textAlign:"center"}}>
        ...
        <div style={{fontSize:16,color:G400}}>Loading…</div>
      </div>
    </div>
  );
    ...
    <div style={{display:"flex",height:"100vh",fontFamily:FONT,background:W,color:BK}}>
      ...
      <div style={{width:252,background:BK,display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto"}}>
        <div style={{padding:"28px 24px 20px"}}>
          ...
          {!isEditor && <div style={{marginTop:10,fontSize:11,background:"#1f1f1f",color:"#737373",padding:"4px 10px",borderRadius:R99,display:"inline-block",fontWeight:600,letterSpacing:"0.05em"}}>👁 View only</div>}
        </div>
        ...
        <div style={{padding:"14px 12px",flex:1}}>
          ...
          {isEditor && <button onClick={()=>setShowNP(true)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 14px",borderRadius:R8,border:"1px dashed #2a2a2a",background:"transparent",color:"#525252",fontSize:13,fontWeight:500,marginTop:8}}>
            <span style={{fontSize:16,lineHeight:1}}>+</span> New project
          </button>}
        </div>
        <div style={{height:1,background:"#1f1f1f"}}/>
        <button onClick={()=>setRole(null)} style={{margin:"12px",padding:"10px",borderRadius:R8,border:"1px solid #2a2a2a",background:"transparent",color:"#525252",fontSize:12,fontWeight:600,cursor:"pointer"}}>← Log out</button>
      </div>

      {/* MAIN */}
      <div style={{flex:1,overflowY:"auto",minWidth:0,background:G50}}>
        {view==="dashboard"&&!activeId && <Dashboard projects={activeProjects} dels={dels} onOpen={id=>{setActiveId(id);setView("project");}} onNew={isEditor?()=>setShowNP(true):null} isEditor={isEditor}/>}
        {view==="project"&&proj && <ProjectView project={proj} deliverables={dels(proj.id)} filter={filter} setFilter={setFilter} expDel={expDel} setExpDel={setExpDel} onDelStatus={isEditor?updateDelStatus:null} onAdd={isEditor?()=>setShowND(true):null} onPhaseChange={isEditor?updateProjectPhase:null} onEdit={isEditor?()=>setEditProject(proj):null} onMarkDone={isEditor?()=>setConfirmDone(proj):null} onDelEdit={isEditor?updateDeliverable:null} onDelDelete={isEditor?deleteDeliverable:null} onEditDel={isEditor?setEditingDel:null} isEditor={isEditor}/>}
        {view==="pipeline" && <PipelineView projects={activeProjects} dels={dels} onOpen={id=>{setActiveId(id);setView("project");}} onPhaseChange={isEditor?updateProjectPhase:null} isEditor={isEditor}/>}
      ...
      {isEditor&&showNP && <NewProjectModal onClose={()=>setShowNP(false)} onSave={d=>{addProject(d);setShowNP(false);}}/>}
      {isEditor&&showND&&activeId && <NewDeliverableModal onClose={()=>setShowND(false)} onSave={d=>{addDeliverable(d);setShowND(false);}}/>}
      {isEditor&&editProject && <EditProjectModal project={editProject} onClose={()=>setEditProject(null)} onSave={d=>{saveProject(editProject.id,d);setEditProject(null);}}/>}
      {isEditor&&confirmDone && <ConfirmDoneModal project={confirmDone} onClose={()=>setConfirmDone(null)} onConfirm={()=>{markProjectDone(confirmDone.id);setConfirmDone(null);}}/>}
      {isEditor&&editingDel && <EditDeliverableModal deliverable={editingDel} onClose={()=>setEditingDel(null)} onSave={data=>{updateDeliverable(editingDel.id,data);setEditingDel(null);}}/>}
      {toast && <Toast msg={toast}/>}
    </div>
  );
}

function Dashboard({ projects, dels, onOpen, onNew, isEditor }) {
  const all=projects.flatMap(p=>dels(p.id));
  const urgent=projects.filter(p=>daysUntil(p.due_date)<=7);
  return (
    <div style={{padding:"44px 52px 64px"}}>
      ...
      {urgent.length>0&&<div style={{background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:R12,padding:"16px 22px",marginBottom:32,display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:18}}>⚠</span><div><div style={{fontSize:14,fontWeight:700,color:"#c2410c",marginBottom:2}}>{urgent.length} project{urgent.length>1?"s":""} due within 7 days</div><div style={{fontSize:13,color:"#9a3412"}}>{urgent.map(p=>p.name).join(" · ")}</div></div></div>}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        {isEditor&&onNew&&<button onClick={onNew} style={{fontSize:14,padding:"11px 22px",borderRadius:R8,border:`1.5px solid ${BK}`,background:BK,color:W,fontWeight:700}}>+ New project</button>}
      </div>
  ...
  return (
    <div onClick={onClick} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow="0 12px 36px rgba(0,0,0,0.09)";}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}} style={{background:W,border:`1px solid ${G200}`,borderRadius:R12,padding:"26px",cursor:"pointer",transition:"all 0.2s",borderTop:`3px solid ${urg?"#ef4444":BK}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
        <div style={{flex:1,marginRight:10}}><div style={{fontSize:16,fontWeight:700,color:BK,marginBottom:4,letterSpacing:"-0.02em",lineHeight:1.3}}>{p.name}</div><div style={{fontSize:13,color:G500,fontWeight:500}}>{p.client}{p.agency?` · ${p.agency}`:""}</div></div>
...
function PipelineView({ projects, dels, onOpen, onPhaseChange, isEditor }) {
  ...
  return (
    <div style={{padding:"44px 40px 64px"}}>
      ...
      <div style={{fontSize:16,color:G500,fontWeight:500,marginBottom:40}}>Projects by phase</div>
      <div style={{display:"grid",gridTemplateColumns:`repeat(${phases.length},1fr)`,gap:12,overflowX:"auto"}}>
        {phases.map(phase=>{
          ...
          return <div key={phase} style={{minWidth:160}}>
            <div style={{background:c.bg,border:`1px solid ${c.border}`,borderRadius:R8,padding:"10px 14px",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <span style={{fontSize:11,fontWeight:700,color:c.text,textTransform:"uppercase",letterSpacing:"0.06em"}}>{phase}</span>
              <span style={{fontSize:13,fontWeight:800,color:c.text}}>{phProjs.length}</span>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {phProjs.map(p=>{
                const pc2=pct(dels(p.id));
                return <div key={p.id} style={{background:W,border:`1px solid ${G200}`,borderRadius:R8,padding:"14px 16px",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 4px 16px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow=""}>
                  <div style={{fontSize:13,fontWeight:700,color:BK,marginBottom:4,lineHeight:1.3}} onClick={()=>onOpen(p.id)}>{p.name}</div>
                  <div style={{fontSize:11,color:G400,marginBottom:10}}>{p.client}</div>
                  <Bar value={pc2}/>
                  <div style={{fontSize:11,color:G400,margin:"6px 0 10px"}}>{pc2}% delivered</div>
                  {isEditor&&onPhaseChange&&<select value={p.phase} onChange={e=>{e.stopPropagation();onPhaseChange(p.id,e.target.value);}} style={{width:"100%",fontSize:11,padding:"5px 8px",border:`1px solid ${G200}`,borderRadius:R4,background:G50,color:G700,fontWeight:600,cursor:"pointer"}}>
...
function ProjectView({ project:p, deliverables, filter, setFilter, expDel, setExpDel, onDelStatus, onAdd, onPhaseChange, onEdit, onMarkDone, onDelEdit, onDelDelete, onEditDel, isEditor }) {
  ...
  return (
    <div style={{padding:"44px 52px 64px"}}>
      <div style={{background:W,border:`1px solid ${G200}`,borderRadius:R12,padding:"36px 40px",marginBottom:24}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,alignItems:"flex-end",flexShrink:0}}>
            ...
            {isEditor&&<div style={{display:"flex",gap:8}}>
              {onEdit&&<button onClick={onEdit} style={{fontSize:13,padding:"8px 16px",borderRadius:R8,border:`1.5px solid ${G200}`,background:W,color:G700,fontWeight:600}}>Edit</button>}
              {onAdd&&<button onClick={onAdd} style={{fontSize:13,padding:"8px 16px",borderRadius:R8,border:"none",background:BK,color:W,fontWeight:600}}>+ Add deliverable</button>}
            </div>}
            {isEditor&&onMarkDone&&<button onClick={onMarkDone} style={{fontSize:13,padding:"9px 18px",borderRadius:R8,border:"1.5px solid #15803d",background:"#f0fdf4",color:"#15803d",fontWeight:700}}>✓ Mark project as Done</button>}
          </div>
        </div>
        {isEditor&&onPhaseChange&&<div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,padding:"14px 18px",background:G50,borderRadius:R8,border:`1px solid ${G200}`}}>
          <span style={{fontSize:12,fontWeight:700,color:G500,textTransform:"uppercase",letterSpacing:"0.07em",whiteSpace:"nowrap"}}>Phase</span>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {PROJECT_PHASES.filter(ph=>ph!=="Archived").map(ph=>{const c=PHASE_COLORS[ph];const isA=p.phase===ph;return <button key={ph} onClick={()=>onPhaseChange(p.id,ph)} style={{fontSize:12,padding:"5px 12px",borderRadius:R4,fontWeight:600,border:`1.5px solid ${isA?c.text:G200}`,background:isA?c.bg:W,color:isA?c.text:G400,transition:"all 0.15s"}}>{ph}</button>;})}
          </div>
        </div>}
        <div style={{display:"flex",alignItems:"center",gap:18,marginBottom:20}}><div style={{flex:1}}><Bar value={pc}/></div><div style={{fontSize:16,fontWeight:700,color:BK,whiteSpace:"nowrap"}}>{deliverables.filter(d=>d.status==="Done").length}/{deliverables.length} · {pc}%</div></div>
        <div style={{height:1,background:G100,marginBottom:18}}/>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{p.team?.map(id=>{const m=getMember(id);return m?<div key={id} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 10px",background:G50,borderRadius:R99,border:`1px solid ${G200}`}}><Avatar memberId={id} size={22}/><span style={{fontSize:13,fontWeight:600,color:G700}}>{m.name}</span></div>:null;})}</div>
      </div>
      ...
      <div style={{background:W,border:`1px solid ${G200}`,borderRadius:R12,overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:`1fr 140px 150px 130px ${isEditor?"90px 40px":""}`,padding:"14px 24px",borderBottom:`1px solid ${G100}`,background:G50}}>
          {["Deliverable","Format","Owner","Status",...(isEditor?["",""]:[])].map((h,i)=><div key={i} style={{fontSize:11,fontWeight:700,color:G400,textTransform:"uppercase",letterSpacing:"0.08em"}}>{h}</div>)}
        </div>
        {filtered.map((d,i)=>{
          const owner=getMember(d.owner);const isExp=expDel[d.id];
          return <div key={d.id}>
            <div style={{display:"grid",gridTemplateColumns:`1fr 140px 150px 130px ${isEditor?"90px 40px":""}`,padding:"16px 24px",borderBottom:i<filtered.length-1?`1px solid ${G100}`:"none",background:d.status==="Done"?"#fafffe":W,alignItems:"center"}}>
              <div>
                ...
                {d.notes&&<div style={{fontSize:12,color:G400,marginTop:3}}>{d.notes}</div>}
              </div>
              ...
              <div><DelBadge status={d.status} onClick={isEditor&&onDelStatus?()=>onDelStatus(d.id,nextDelStatus(d.status)):null}/></div>
              {isEditor&&<div style={{display:"flex",gap:4}}>
                ...
                {onEditDel&&<button onClick={()=>onEditDel(d)} style={{fontSize:11,padding:"5px 8px",borderRadius:R4,border:`1px solid ${G200}`,background:"transparent",color:G400,fontWeight:600}}>✎</button>}
              </div>}
              {isEditor&&<div>{onDelDelete&&<button onClick={()=>onDelDelete(d.id)} style={{fontSize:13,padding:"4px 6px",borderRadius:R4,border:"none",background:"transparent",color:G300,fontWeight:700}}>×</button>}</div>}
            </div>
            {isExp&&d.delivery&&<div style={{padding:"16px 24px 20px",background:G50,borderBottom:i<filtered.length-1?`1px solid ${G100}`:"none"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:12}}>{DELIVERY_SPECS.map(s=><div key={s} style={{display:"flex",alignItems:"center",gap:8,fontSize:14,color:G700,fontWeight:500}}><div style={{width:18,height:18,border:`2px solid ${G300}`,borderRadius:4,background:W,flexShrink:0}}/>{s}</div>)}</div>
            </div>}
          </div>;
        })}
  ...
  return (
    <div style={{padding:"44px 52px 64px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:16}}>
        {wl.map(m=>{const pc2=m.total?Math.round(m.done/m.total*100):0;return(
          <div key={m.id} style={{background:W,border:`1px solid ${G200}`,borderRadius:R12,padding:"26px"}}>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:17,fontWeight:700,color:BK,letterSpacing:"-0.02em"}}>{m.name}</div><div style={{fontSize:13,color:G500,marginTop:3}}>{m.role}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:40,fontWeight:800,color:m.todo>0?BK:"#15803d",letterSpacing:"-0.04em",lineHeight:1}}>{m.todo}</div><div style={{fontSize:11,color:G400,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em"}}>pending</div></div>
            </div>
            <Bar value={pc2}/>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {m.items.slice(0,5).map(d=>{const c=DEL_STATUS_COLORS[d.status]||DEL_STATUS_COLORS["Not started"];return(
                <div key={d.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",background:G50,borderRadius:R8}}>
                  <div style={{flex:1,marginRight:10}}><div style={{fontSize:13,color:G700,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.label}</div><div style={{fontSize:11,color:G400,marginTop:2}}>{d.projectName}</div></div>
      ...
      {projects.length===0
        ...
        :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16}}>
          {projects.map(p=><div key={p.id} onClick={()=>onOpen(p.id)} style={{background:W,border:`1px solid ${G200}`,borderRadius:R12,padding:"26px",opacity:0.75,cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={e=>e.currentTarget.style.opacity="1"} onMouseLeave={e=>e.currentTarget.style.opacity="0.75"}>
            <div style={{fontSize:16,fontWeight:700,color:BK,marginBottom:5}}>{p.name}</div>
...
function NewDeliverableModal({ onClose, onSave }) {
  ...
  return <Modal title="Add deliverable" onClose={onClose}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}><input type="checkbox" id="dc" checked={f.delivery} onChange={e=>s("delivery",e.target.checked)} style={{width:20,height:20,accentColor:BK}}/><label htmlFor="dc" style={{fontSize:15,color:G700,fontWeight:500,cursor:"pointer"}}>Requires full delivery package</label></div>
...
function EditDeliverableModal({ deliverable, onClose, onSave }) {
  ...
  return <Modal title="Edit deliverable" onClose={onClose}>
    ...
    <div style={FS}><label style={LS}>Owner</label><select style={IS} value={f.owner} onChange={e=>s("owner",e.target.value)}>{TEAM.map(m=><option key={m.id} value={m.id}>{m.name} — {m.role}</option>)}</select></div>
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}><input type="checkbox" checked={f.delivery} onChange={e=>s("delivery",e.target.checked)} style={{width:20,height:20,accentColor:BK}}/><span style={{fontSize:15,color:G700,fontWeight:500}}>Requires full delivery package</span></div>
...
function ConfirmDoneModal({ project, onClose, onConfirm }) {
  return <Modal title="Mark project as Done?" onClose={onClose}>
    <div style={{fontSize:15,color:G500,marginBottom:24,lineHeight:1.7}}>This will mark <strong style={{color:BK}}>{project.name}</strong> as complete, set all deliverables to Done, and move it to the Archive.</div>
